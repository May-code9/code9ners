  ...
  <div class="container">
    ...
    <div class="row">
      <div class="col-sm-16 col-md-12 push-1">
        <div class="card">
          <div class="card-header">
            <h5 class="card-title pull-left">All Website Features | <small>Total Number of Websites Features: </small> {{ noTrashedSiteFeature() }} </h5>
            ...
            <table class="table " id="dataTables-example">
              ...
              <tbody>
                ...
                <tr class="">
                  ...
                  <td class="center">
                    <a href="/siteFeatureTrash/{{$getFeature->id}}" onclick="event.preventDefault(); document.getElementById('{{$getFeature->id}}').submit();" class="btn btn-sm btn-primary mr-2">Restore</a>
                    <form id="{{$getFeature->id}}" action="/siteFeatureTrash/{{ $getFeature->id }}" method="POST" style="display: none;">
                      {{ method_field('PUT') }}
                      {{ csrf_field() }}
                    </form>
                  </td>
                  <td class="center">
                    <a href="/siteFeatureTrash/{{ $getFeature->id }}" onclick="event.preventDefault(); document.getElementById('{{$getFeature->website_feature}}').submit();" class="btn btn-sm btn-danger mr-2">Delete</a>
                    <form id="{{$getFeature->website_feature}}" action="/siteFeatureTrash/{{ $getFeature->id }}" method="POST" style="display: none;">
                      {{ method_field('DELETE') }}
                      {{ csrf_field() }}
                    </form>
                  </td>
